...
class DescargaEncabezadosController extends AppController
{
    ...
    public function listarDisponibles($recursoId)
    {
		...
		$totalDescargas = $this->loadModel('DescargasDisponibles')->find('all', [
			'contain' => [ 'Recaladas.Mareas',
		...
		while ($preValue < $totalDescargas) {
        $tmpDescargas = $this->loadModel('DescargasDisponibles')->find('all', [
			'limit' => 1000,
			'offset' => $preValue,
			...
			if ($descargas == null) {
				$descargas = $tmpDescargas->all();
			} else {
				$descargas = $descargas->append($tmpDescargas->all());
			}
		}
