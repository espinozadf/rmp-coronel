...
class LicenciasController extends AppController {
      ...
      $divisionUsuario = $this->divisionUsuario($user);
        ...
        if ($this->request->is(['get'])) {
            ...
            if ($licenciasYear->count() == 0) {
                ...
                $firstYear = $licenciasYear->cleanCopy()->order('fecha_inicio_vigencia ASC')->first()->toArray()['fecha_inicio_vigencia']->format('Y');
                $lastYear = $licenciasYear->order('fecha_inicio_vigencia DESC')->first()->toArray()['fecha_inicio_vigencia']->format('Y');
            }

            // $years = range($lastYear, $firstYear);
            $years = range(2024, 2020);
 
            $this->set(['years' => $years]);
        }

        if ($this->request->is(['ajax', 'get'])) {
             $licencias = $this->Licencias->find('all',[ 'conditions' => ['division IN' => $divisionUsuario]]);
            
            // $licencias = $this->Licencias->find('all')->where(['fecha_inicio_vigencia >=' => '2019-01-01']);
            ...
            if ($this->request->query('year')) {
                $licencias = $licencias
                ->where([
                    'YEAR(fecha_inicio_vigencia) IN' => explode(',', $this->request->query('year'))
                ]);
            }
    ...
    //         'contain' => [
    //             'TiposLicencia', 'TipoOrigen', 'ModificaLicencias',
    //             'Auxiliares', 'Especies', 'MacroZonas','Naves',
    //             'Unidades', 'Numeraciones', 'UnidadesCosto','Divisiones']
    //     ]);
    ...
    //     if ($licencia->tipo_licencia_id == 4  || $licencia->tipo_licencia_id == 34) { // RAE carga la nave
