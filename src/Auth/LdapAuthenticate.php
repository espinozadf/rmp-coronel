...
class LdapAuthenticate extends BaseAuthenticate
{
    public function authenticate(Request $request, Response $response)
    {
      ...
      $adServer = "ldap://192.168.209.2"; // se conecta al servidor MS Directory correspondiente
  //      $adServer = "ldap://192.168.1.201";
	$ldapConnection = @ldap_connect($adServer);
      ...
      $ldaprdn = 'CAMANCHACA' . "\\" . $username;
      
      // $bind = @ldap_bind($ldapConnection, 'CAMANCHACA_STGO\\'.$username, $password);
      $bind = @ldap_bind($ldapConnection, 'CAMANCHACA\\'.$username, $password);
